...
Installing packages
```{r, chunk-label, results='hide',cache=TRUE}


x <- c("ggmap", "rgdal", "rgeos", "maptools", "dplyr", "tidyr", "rworldmap", "knitr", "plm", "SpatioTemporal", "maps","xts", "spacetime", "vardiag")

lapply(x, function(x) if(x %in% rownames(installed.packages()) == FALSE) {install.packages(x)})
...
opts_knit$set(root.dir = '/Users/faustofabiancrespofernandez/Downloads/ResidualAnalysis/Rcode/')
setwd("/Users/faustofabiancrespofernandez/Downloads/ResidualAnalysis/Rcode/")
sismoscompleto <- read.csv(paste(getwd(),"/sismos5.csv",sep=""), header=T, stringsAsFactors=FALSE, fileEncoding="latin1")
mainschock = sismoscompleto[sismoscompleto$SegundosDesdeTerr16abril==0,]


sismoscompleto$Profundidad2<-suppressWarnings(as.numeric(sismoscompleto$Profundidad))

sismoscompleto<- sismoscompleto[!is.na(sismoscompleto$Profundidad2),]

sismoscompleto = sismoscompleto[complete.cases(sismoscompleto$Profundidad2),]

sismoscompleto = sismoscompleto[!duplicated(sismoscompleto),]
nrow(sismoscompleto)#896
sismoscompleto$Ciudad.mas.cercana2<- substr(sismoscompleto$Ciudad.mas.cercana, regexpr('km', sismoscompleto$Ciudad.mas.cercana)+3,  regexpr(',', sismoscompleto$Ciudad.mas.cercana)-1)
sismoscompleto$Provincia<-substr(sismoscompleto$ProvinciaMasCercana,  regexpr(',', sismoscompleto$ProvinciaMasCercana)+1, nchar(sismoscompleto$ProvinciaMasCercana))

sismosManabiEsmeraldas=sismoscompleto[sismoscompleto$ProvinciaMasCercana %in% c("Manabi","Esmeraldas"),]
nrow(sismosManabiEsmeraldas)#803


length(which(sismoscompleto$Magnitud>3.5))#577
length(which(sismoscompleto$Magnitud>3.0))#865
length(which(sismoscompleto$Magnitud>=3.0))#892
table(sismoscompleto$Mes2)
#abril julio junio marzo  mayo 
#  619    56    51    10   160
table(sismosManabiEsmeraldas$Mes2,sismosManabiEsmeraldas$Provincia)


df <- data.frame(x = sismoscompleto$LongitudValor, y = sismoscompleto$LatitudValor)
df
mat <- as.matrix(df) # create matrix object with as.matrix
sp1 <- SpatialPoints(coords = mat)
sp1

spdf <- SpatialPointsDataFrame(sp1, data = sismoscompleto)
#View(spdf)
...
```
```{r, echo=FALSE}
color <- rep("xx", nrow(sismos.ecuador@data))
color[(sismos.ecuador@data$Magnitud > 0)&(sismos.ecuador@data$Magnitud <= 5)] <- "blue"
color[(sismos.ecuador@data$Magnitud  > 5)&(sismos.ecuador@data$Magnitud  <= 6)] <- "yellow"
color[(sismos.ecuador@data$Magnitud  > 6)&(sismos.ecuador@data$Magnitud  <= 7)] <- "orange"
color[(sismos.ecuador@data$Magnitud   > 7)] <- "red"
# color <- sismos.ecuador$Magnitud
# ( color <- ifelse(color > 0 & color <= 5, "blue",
#                   ifelse(color > 5 & color <= 6, "yellow",
#                       ifelse(color > 6 & color <= 7, "orange",
#                          ifelse(color > 7, "red", NA))) )  
#library(rworldmap)
#newmap <- getMap(resolution = "low")
plot(newmap,  xlim = c(-85, -77), ylim = c(-6,2), asp = 1)
points(sismos.ecuador@data$LongitudValor,sismos.ecuador@data$LatitudValor,col=color,cex = .6, pch=19)
#legend("bottomleft", legend=names(attr(color, "table")), fill=attr(color, "palette"), bty="n")
legend("bottomleft", legend=c("<5","5-6","6-7",">7"),
       fill=c("blue", "yellow", "orange", "red"), bty="n")

title(main="Earthquake Ecuador March 18th,2016-July 16th,2016 
      (magnitude)")
...
( cuts <- classIntervals(sismos.ecuador@data$Magnitud , style="fixed", fixedBreaks=c(summary(sismos.ecuador@data$Magnitud))) )
plotclr <- rev(brewer.pal(5, "Spectral"))
colcode <- findColours(cuts, plotclr)


plot(newmap,  xlim = c(-85, -77), ylim = c(-6,2), asp = 1)
points(sismos.ecuador@data$LongitudValor,sismos.ecuador@data$LatitudValor,col=colcode,cex = .6, pch=19)
#plot(sismos.ecuador, col=colcode, pch=19)
box()
legend("bottomleft", legend=c("2.5-3.4","3.4-3.7","3.7-3.826","3.826-4.1","4.1-7.4"),
       fill=attr(colcode, "palette"), bty="n")
title(main="Earthquake Ecuador March 18th,2016-July 16th,2016")
```
```{r, echo=FALSE}

mainshock<-sismoscompleto[sismoscompleto$Magnitud==max(sismoscompleto$Magnitud),]
plot(sismoscompleto$SegundosDesdeTerr16abril/(24*3600),sismoscompleto$Magnitud,xlab="Time from mainshock(days)",ylab="Magnitude",pch = 20,log="x")
title(main="Earthquake Magnitudes April 16th,2016-July 16th,2016")
  abline(v=1, lwd=1, lty=2)
  abline(h=3, lwd=1, lty=2)
  points(1, mainshock$Magnitud,col="red", pch = 19)
```
 
```{r, echo=FALSE}
 times=sismoscompleto$DiasDesde16abril
 magnitudes=sismoscompleto$Magnitud
 par(mar = c(5, 4, 4, 4) + 0.3)
plot(times, magnitudes, type="h", xlab="Time (days)", ylab="Magnitude")
# #title(main="Magnitud Sismos 18/03/2016-16/07/2016 \n(escala d?as desde 16 de abril)")

 par(new = TRUE)
event.indices <- 1:length(times)
plot(sort(times), as.numeric(event.indices), type="l", axes=FALSE, bty = "n",
        xlab = "", ylab = "")
 axis(side=4, at = pretty(range(as.numeric(event.indices))))
 mtext("Cumulated number of events ", side=4, line=3)
title(main="Earthquake magnitudes March 18th, 2016-July 16th, 2016")
```
```{r, echo=FALSE}

times1=sort(times[times>=0])
times1=times1[3:length(times1)]
first.time <- times1[1]
event.time <- times1[2:length(times1)]

 mean.rate <- NULL
event.number <- NULL
for(i in 1:(length(event.time)))
   {
    event.number <- c(event.number, i)
     }
mean.rate <- event.number/(event.time-first.time)
plot(times1[2:length(times1)], mean.rate, type="l", xlab="Time(days)", ylab="Mean daily event rate")
title(main="Mean daily event rate April 16th, 2016-July 16th, 2016")
```
...
descriptive<-as.data.frame(table(sismoscompleto$Provincia,sismoscompleto$Mes2 ))
colnames(descriptive)<-c("Province", "Month", "Count")
sum(descriptive$Count[descriptive$Month=="julio"])
sum(descriptive$Count[descriptive$Month=="abril"])
sum(descriptive$Count[descriptive$Month=="junio"])
sum(descriptive$Count[descriptive$Month=="mayo"])
sum(descriptive$Count[descriptive$Month=="marzo"])
```


```{r, echo=FALSE, cache=FALSE}

sismos1=sismosManabiEsmeraldas[,c("fila","Mes","dia","hora","minuto","LongitudValor","LatitudValor","Magnitud", "SegundosDesdeTerr16abril")]
...
sismos1$tiempodesdeAhora<-difftime(rep(.POSIXct(Sys.time(), tz="America/Guayaquil"), nrow(sismos1)),
                                  as.POSIXct(paste(sismos1$date," ",sismos1$time),"%Y-%m-%d %H:%M:%S",tz="America/Guayaquil")
                                  ,units="secs")
...
sismos1$tiempo.dias<-difftime(as.POSIXct(paste(sismos1$date," ",sismos1$time), "%Y-%m-%d %H:%M:%S",tz="America/Guayaquil"),
                                 as.POSIXct(paste(sismos1[indice.sismo.mas.antiguo,]$date," ",sismos1[indice.sismo.mas.antiguo,]$time),"%Y-%m-%d %H:%M:%S",tz="America/Guayaquil")
                                  ,units="days")


sismos1<-sismos1[with(sismos1, order(-tiempodesdeAhora)), ]
#sismos1<-sismos1[with(sismos1, order(SegundosDesdeTerr16abril)), ]

sismos1$fila<-seq.int(nrow(sismos1))
...
ecuador.cat <- catalog(sismos2, time.begin="2016/04/13",
                       study.start="2016/04/13", study.end="2016/07/16",lat.range=c(min(sismos2$lat), max(sismos2$lat)), long.range=c(min(sismos2$long), max(sismos2$long)), mag.threshold=3)
plot(ecuador.cat)
param01 <- c(0.46, 0.23, 0.022, 2.8, 1.12, 0.012, 2.4, 0.35)


ecuador.fit <- etas(ecuador.cat, param0=param01, plot.it = FALSE,no.itr=1)
...
library(ETAS)
sismosnuevos <- read.csv("/Users/faustofabiancrespofernandez/Downloads/ResidualAnalysis/Rcode/sismos4.csv",  header=T, stringsAsFactors=FALSE, fileEncoding="latin1")

sismosnuevos$Provincia<-substr(sismosnuevos$ProvinciaMasCercana,  regexpr(',', sismosnuevos$ProvinciaMasCercana)+1, nchar(sismosnuevos$ProvinciaMasCercana))

sismosManabiEsmeraldasNuevo=sismosnuevos[sismosnuevos$Provincia %in% c("Manabi","Esmeraldas"),]
...
sismos6$tiempodesdeAhora<-difftime(rep(.POSIXct(Sys.time(), tz="America/Guayaquil"), nrow(sismos6)),
                                  as.POSIXct(paste(sismos6$date," ",sismos6$time),"%Y-%m-%d %H:%M:%S",tz="America/Guayaquil")
                                  ,units="secs")
...
sismos7$tiempo.dias<-difftime(as.POSIXct(paste(sismos7$date," ",sismos7$time),"%Y-%m-%d %H:%M:%S", tz="America/Guayaquil"),
                                  as.POSIXct(fechaMasantiguo,"%Y-%m-%d %H:%M:%S",tz="America/Guayaquil")
                                  ,units="days")
...
View(sismos7)
```
 Filtering the data to remove NAs, missing data and to get the events near Manabi and Esmeraldas provinces.
```{r,echo=FALSE, cache=TRUE}
library(rstan)
library(BH)
library(StanHeaders)
...
# sismoscompleto <- read.csv(paste("/Users/faustofabiancrespofernandez/Downloads/ResidualAnalysis/Rcode/","/sismos5.csv",sep=""), header=T, stringsAsFactors=FALSE, fileEncoding="latin1")
# sismoscompleto$Profundidad2<-suppressWarnings(as.numeric(sismoscompleto$Profundidad))
# 
# sismoscompleto<- sismoscompleto[!is.na(sismoscompleto$Profundidad2),]
# sismoscompleto = sismoscompleto[complete.cases(sismoscompleto$Profundidad2),]
...
# 
# sismosManabiEsmeraldas=sismoscompleto[sismoscompleto$ProvinciaMasCercana %in% c("Manabi","Esmeraldas"),]
#sismoscompleto = sismoscompleto[!duplicated(sismoscompleto$SegundosDesdeTerr16abril),]

#sismoscompleto = sismoscompleto[complete.cases(sismoscompleto$SegundosDesdeTerr16abril),]
#sismoscompleto = sismoscompleto[complete.cases(sismoscompleto$Profundidad2),]
...
# sismos1<-sismoscompleto[,c("Magnitud","LongitudValor","LatitudValor","Profundidad2","Mes","DiasDesde16abril" )]
# 
# sismosManabiEsmeraldas=sismoscompleto[sismoscompleto$ProvinciaMasCercana %in% c("Manabi","Esmeraldas"),]
...
sismos$tiempodesdeAhora<-difftime(rep(.POSIXct(Sys.time(), tz="America/Guayaquil"), nrow(sismos)),
                                  as.POSIXct(paste(sismos$date," ",sismos$time),"%Y-%m-%d %H:%M:%S",tz="America/Guayaquil")
                                  ,units="secs")
...
while(nrow(sismos[sismos$escogido == FALSE,]) > 0)
{
 ...
 tiempo.sismo.max.mag=as.POSIXct(paste(sismos[indice.mayor.sismo,]$date," ",sismos[indice.mayor.sismo,]$time),"%Y-%m-%d %H:%M:%S",tz="America/Guayaquil")
 ...
 indices=which(sismos$escogido==FALSE &  
                 difftime(as.POSIXct(paste(sismos$date," ",sismos$time),"%Y-%m-%d %H:%M:%S",tz="America/Guayaquil")
...
length(which(sismos$cluster==1))#797
 print("Number  of clusters:")
n=max(sismos$cluster)#6
summary(sismos$EsCentroide)
centroides=sismos[sismos$EsCentroide==1,]
nrow(centroides)
```
...
sismos$tiempo.minutos=difftime(as.POSIXct(paste(sismos$date," ",sismos$time),"%Y-%m-%d %H:%M:%S",tz="America/Guayaquil")
                                 ,as.POSIXct(paste(sismos[indice.sismo.mas.antiguo,]$date," ",sismos[indice.sismo.mas.antiguo,]$time),"%Y-%m-%d %H:%M:%S",tz="America/Guayaquil")
                                 ,units="mins")
sismos$tiempo.dias=difftime(as.POSIXct(paste(sismos$date," ",sismos$time),"%Y-%m-%d %H:%M:%S",tz="America/Guayaquil")
                               ,as.POSIXct(paste(sismos[indice.sismo.mas.antiguo,]$date," ",sismos[indice.sismo.mas.antiguo,]$time),"%Y-%m-%d %H:%M:%S",tz="America/Guayaquil")
                               ,units="days")
...
```{r}
library("devtools")
if("vardiag" %in% rownames(installed.packages()) == FALSE){
    devtools::install_github("edzer/vardiag")
}
 if("geoR" %in% rownames(installed.packages()) == FALSE){
     install_github("cran/geoR")
 }

if("sn" %in% rownames(installed.packages()) == FALSE){
    install_github("cran/sn")
}
 if("mclust" %in% rownames(installed.packages()) == FALSE){
     install_github("cran/mclust")
 }   
library(vardiag)
library(geoR)#perhaps you need to install xquartz first
...
library(sn)
library(mclust)#requires gfortran
...
```{r}
library("devtools")
if("shotGroups" %in% rownames(installed.packages()) == FALSE){
    install_github("cran/shotGroups")
}
library(shotGroups)
sismosCluster<-sismos[sismos$cluster==1,]
xy<- data.frame(x = sismosCluster$LongitudValor, y =sismosCluster$LatitudValor)
circle<-getMinCircle(xy)
circle$ctr
circle$rad

 mainshock<-sismosCluster[sismosCluster$Magnitud==max(sismosCluster$Magnitud),]
centro=circle$ctr
par(pty= "s")
dev.off()
plot(newmap,  xlim = c(-83, -80), ylim = c(-2,2), asp = 1)
#points(ellipse::ellipse(mc,centre=centro, npoints = 1000,level = 0.985),type = 'l',col="red")
points(drawCircle(centro, radius=circle$rad))
points(sismosCluster$LongitudValor,sismosCluster$LatitudValor,cex = .6, pch=19)
points(centro[1],centro[2], cex = .6, pch=19,col="red")
points(mainshock$LongitudValor,mainshock$LatitudValor, cex = .6, pch=19,col="green")
title(main=" Minimum Covering Circle for main cluster")
```
...
```{r}
  ...
  r=circle$rad#1.704791211927978
...
rstan_options(auto_write = TRUE)
fit = stan(file="/Users/faustofabiancrespofernandez/Downloads/ResidualAnalysis/Rcode/spETAStiempos2.stan", data=dat, iter=500,  chains=4,refresh=1,control = list(adapt_delta = 0.9999))#este tambien
```
...
library("devtools")
  ...
  lib <- c("rworldmap", "ggmap", "rgdal", "rgeos", "maptools", "dplyr", "tidyr")
 lapply(lib, FUN=function(x)   if(x %in% rownames(installed.packages()) == FALSE){
    devtools::install_github(paste("cran/", x))
})  

  lapply(lib, library, character.only = TRUE) # load the required packages

  if("ellipse" %in% rownames(installed.packages()) == FALSE){
    install_github("cran/ellipse")
}
library(ellipse)
