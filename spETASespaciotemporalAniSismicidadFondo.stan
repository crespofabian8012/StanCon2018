functions{
  real loglikelihood(int N,
    ...
    for(j in 1:(N-1)){
      ...
      y<-y .* z;
      tasa_sismicidad[j]<-log(mu*tasas_sismicidad_fondo[j+1]+sum(y));
    ...
    integral_mu<-mu*sum(tasas_sismicidad_fondo)*tmax*pi()*radio^2;
...
model{
 ...
 d~exponential(2.8);
  ...
  increment_log_prob(loglikelihood(N,mu,k,p,c,q,d,alpha,gamma,tiempo,magnitudes,dif_tiempos,latitudes,longitudes,tasas_sismicidad_fondo,factor_cuadraticoAni,factor_cuadraticoIso,tiempo_max,magnitud_corte,lat_min,lat_max,long_min,long_max,radio));
}